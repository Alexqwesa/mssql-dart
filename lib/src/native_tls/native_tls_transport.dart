...
final class NativeTlsTransport {
  ...
  Future<void> writePacket(Uint8List packet) async {
    ...
    var retriedAfterDrain = false;
    while (result != 0) {
      await _drainCiphertext();
      if (result != 1) {
        throw StateError('Native TLS packet write failed with code $result.');
      }
      // OpenSSL can report WANT_READ while ciphertext generated by this write
      // is still queued in the memory BIO. Retry once after draining it before
      // waiting for peer input; Attention must not deadlock here.
      if (!retriedAfterDrain) {
        retriedAfterDrain = true;
        result = _engine.retryWrite();
        continue;
      }
      final ready = _inputReady ??= Completer<void>();
      await ready.future;
      retriedAfterDrain = false;
      result = _engine.retryWrite();
    }
    await _drainCiphertext();
  }
